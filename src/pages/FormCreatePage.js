...
const FormCreatePage = () => {
  ...
  const onUpdateOption = (questionIndex, optionIndex) => (e) => {
    dispatch(
      updateOption({ questionIndex, optionIndex, value: e.target.value })
    );
  };
  ...
  return (
    <>
      <Container>
        <FloatingMenu>
          <MenuButton title="Add New Question" onClick={onAddQuestion}>
            <AddCircleOutlineIcon />
          </MenuButton>
          <BreakLine />
          <MenuButton title="Save Form" onClick={onCreateForm}>
            <SaveOutlinedIcon />
          </MenuButton>
        </FloatingMenu>
        <FormHeadingCard
